...
@pytest.fixture
def qmsgbox(monkeypatch):
    """Patch all QMessageBox functions to immediately return a given value.

    Note: this does not affect QMessageBoxes generated by the Qt C++ code, only
    ones created from Python.

    """
    from qtpy.QtWidgets import QMessageBox

    monkeypatch.setattr(QMessageBox, "about", lambda *_: None)
    monkeypatch.setattr(QMessageBox, "aboutQt", lambda *_: None)
    monkeypatch.setattr(QMessageBox, "critical", lambda *_: QMessageBox.Ok)
