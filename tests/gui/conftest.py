...
from qtpy.QtCore import QDir, Qt, QTimer
...
from tests.gui.simulation.test_run_path_dialog import handle_run_path_dialog
...
def qapp_cls():
    """Overload pytest-qt's qapp_cls to provide additional functionality"""

    def init(*args):
        qapp = QApplication(*args)

        next_excepthook = sys.excepthook
        next_sighandler = getsignal(SIGINT)

        def reset():
            sys.excepthook = next_excepthook
            signal(SIGINT, next_sighandler)
        ...
        def sighandler(signum, frame):
            qapp.exit(-signum)
            reset()
            return next_sighandler(signum, frame)
